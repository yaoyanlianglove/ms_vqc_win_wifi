...
void YZTY::ui_init()
{
    ...
    QStringList yk_list;
    yk_list<<QString::fromLocal8Bit("升压")<<QString::fromLocal8Bit("降压")<<QString::fromLocal8Bit("投切电容");
    ...
    QStringList cap1_list;
    cap1_list<<QString::fromLocal8Bit("投")<<QString::fromLocal8Bit("切");
    ui->cap1_select->addItems(cap1_list);
    ui->cap2_select->addItems(cap1_list);
...
void YZTY::com_set()
{
    port_info = new QSerialPortInfo(ui->comboBox_num_com->currentText());
...
void YZTY::heart_handle(int len)
{
    ...
    QString cap1_num = QString::number((readData[20] << 8) + readData[21]);
    ui->cap1_num_stat->setText(cap1_num);

    QString cap2_num = QString::number((readData[22] << 8) + readData[23]);
    ui->cap2_num_stat->setText(cap2_num);

    if(readData[19] == 0)
        ui->cap_stat->setText(QString::fromLocal8Bit("0-0"));
    else if(readData[19] == 1)
        ui->cap_stat->setText(QString::fromLocal8Bit("0-1"));
    else if(readData[19] == 2)
        ui->cap_stat->setText(QString::fromLocal8Bit("1-0"));
    else if(readData[19] == 3)
        ui->cap_stat->setText(QString::fromLocal8Bit("1-1"));
    ...
    if((readData[15] & 0x20) == 0x20)
        ui->cap1_fault->setChecked(true);
    else
        ui->cap1_fault->setChecked(false);

    if((readData[15] & 0x40) == 0x40)
        ui->cap2_fault->setChecked(true);
    else
        ui->cap2_fault->setChecked(false);
    ...
    if((readData[14] & 0x20) == 0x20)
        ui->turn_vol_fail->setChecked(true);
    else
        ui->turn_vol_fail->setChecked(false);
    if((readData[14] & 0x40) == 0x40)
        ui->cap_control_fail->setChecked(true);
    else
        ui->cap_control_fail->setChecked(false);
...
void YZTY::on_hand_input_code_clicked()
{
    ...
    if((alarmCode & 0x800) == 0x800)
        ...
        ui->oil_pos_alarm->setChecked(false);
    if((alarmCode & 0x1000) == 0x1000)
        ui->oil_tmp_alarm->setChecked(true);
    else
        ui->oil_tmp_alarm->setChecked(false);

    if((alarmCode & 0x2000) == 0x2000)
        ui->turn_vol_fail->setChecked(true);
    else
        ui->turn_vol_fail->setChecked(false);
...
void YZTY::yc_handle(int len)
{
    ...
    ui->textEdit_print_ts->append(s);
    print(len, readData, 0);
    float u_a = ((readData[6]<<8) + readData[7]);
    ui->lineEdit_u_a_yc->setText(QString::number(u_a));

    float u_b = ((readData[8]<<8) + readData[9]);
    ui->lineEdit_u_b_yc->setText(QString::number(u_b));

    float u_c = ((readData[10]<<8) + readData[11]);
    ui->lineEdit_u_c_yc->setText(QString::number(u_c));

    float i_a = ((readData[12]<<8) + readData[13])/10.0;
    ui->lineEdit_i_a_yc->setText(QString::number(i_a));

    float i_b = ((readData[14]<<8) + readData[15])/10.0;
    ui->lineEdit_i_b_yc->setText(QString::number(i_b));

    float i_c = ((readData[16]<<8) + readData[17])/10.0;
    ui->lineEdit_i_c_yc->setText(QString::number(i_c));

    float p_a = ((qint16)((readData[18]<<8) + readData[19]))/10.0;
    ui->lineEdit_p_a_yc->setText(QString::number(p_a));

    float p_b = ((qint16)((readData[20]<<8) + readData[21]))/10.0;
    ui->lineEdit_p_b_yc->setText(QString::number(p_b));

    float p_c = ((qint16)((readData[22]<<8) + readData[23])/10.0);
    ui->lineEdit_p_c_yc->setText(QString::number(p_c));

    float q_a = ((qint16)((readData[24]<<8) + readData[25]))/10.0;
    ui->lineEdit_q_a_yc->setText(QString::number(q_a));
    float q_b = ((qint16)((readData[26]<<8) + readData[27]))/10.0;
    ui->lineEdit_q_b_yc->setText(QString::number(q_b));
    float q_c = ((qint16)((readData[28]<<8) + readData[29]))/10.0;
...
void YZTY::send_yk_switch()
{
    ...
    {
        if(ui->cap1_select->currentIndex() == 0 && ui->cap2_select->currentIndex() == 0)
            sendData[6] = 0x33;    //投切电容
        else if(ui->cap1_select->currentIndex() == 1 && ui->cap2_select->currentIndex() == 0)
            sendData[6] = 0x23;    //投切电容
        else if(ui->cap1_select->currentIndex() == 0 && ui->cap2_select->currentIndex() == 1)
            sendData[6] = 0x13;    //投切电容
        else if(ui->cap1_select->currentIndex() == 1 && ui->cap2_select->currentIndex() == 1)
            sendData[6] = 0x03;    //投切电容
    }
...
void YZTY::get_device_cfg_handle(int len)
{
    ...
    int bh_oil_tmp   = (readData[26] << 8) + readData[27];
    int tran_content = (readData[28] << 8) + readData[29];
    int dev_code     = (readData[30] << 8) + readData[31];
    int pf_up        = (readData[32] << 8) + readData[33];
    int pf_down      = (readData[34] << 8) + readData[35];
    int q_need       = (readData[36] << 8) + readData[37];
    int tc_delay     = (readData[38] << 8) + readData[39];
    int qc_delay     = (readData[40] << 8) + readData[41];
    int switch_total = (readData[42] << 8) + readData[43];
    int tran_range_h = (readData[44] << 8) + readData[45];
    int tran_range_l = (readData[46] << 8) + readData[47];
    int switch_type  = (readData[48] << 8) + readData[49];
    int dev_h_v      = (readData[50] << 8) + readData[51];
    int dev_s_v      = (readData[52] << 8) + readData[53];
    ...
    ui->pf_up->setText(QString::number(pf_up));
    ui->pf_down->setText(QString::number(pf_down));
    ui->q_need->setText(QString::number(q_need));
    ui->tc_delay->setText(QString::number(tc_delay));
    ui->qc_delay->setText(QString::number(qc_delay));
    ...
    else if(switch_type == 0x05)
        ui->switch_type->setText(QString::fromLocal8Bit("VQC双直流电机调压开关17档"));
...
void YZTY::send_set_device_cfg()
{
    ...
    sendData[26] = ui->bh_oil_tmp->text().toInt() >> 8;
    sendData[27] = ui->bh_oil_tmp->text().toInt() &  0xFF;
    sendData[28] = ui->tran_content->text().toInt() >> 8;
    sendData[29] = ui->tran_content->text().toInt() &  0xFF;
    sendData[30] = ui->dev_code->text().toInt() >> 8;
    sendData[31] = ui->dev_code->text().toInt() & 0xFF;
    sendData[32] = ui->pf_up->text().toInt() >> 8;
    sendData[33] = ui->pf_up->text().toInt() & 0xFF;
    sendData[34] = ui->pf_down->text().toInt() >> 8;
    sendData[35] = ui->pf_down->text().toInt() & 0xFF;
    sendData[36] = ui->q_need->text().toInt() >> 8;
    sendData[37] = ui->q_need->text().toInt() & 0xFF;
    sendData[38] = ui->tc_delay->text().toInt() >> 8;
    sendData[39] = ui->tc_delay->text().toInt() & 0xFF;
    sendData[40] = ui->qc_delay->text().toInt() >> 8;
    sendData[41] = ui->qc_delay->text().toInt() & 0xFF;
...
void YZTY::get_ty_rec_handle(int len)
{
    ...
    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[16]<<8) + readData[17]));
    QString ua_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[18]<<8) + readData[19]));
    QString ub_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[20]<<8) + readData[21]));
    QString uc_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[22]<<8) + readData[23])/10.0);
    QString ia_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[24]<<8) + readData[25])/10.0);
    QString ib_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[26]<<8) + readData[27])/10.0);
    QString ic_buf = temp;
    ...
    QString gear_buf = QString::number((readData[29] >> 1) & 0x1f);

    QString cap_buf;
    int cap_num = readData[29] >> 6;

    if(cap_num == 0)
        cap_buf = QString::fromLocal8Bit("电容1：未投-电容2：未投");
    else if(cap_num == 1)
        cap_buf = QString::fromLocal8Bit("电容1：已投-电容2：未投");
    else if(cap_num == 2)
        cap_buf = QString::fromLocal8Bit("电容1：未投-电容2：已投");
    else if(cap_num == 3)
        cap_buf = QString::fromLocal8Bit("电容1：已投-电容2：已投");

    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[30]<<8) + readData[31]));
    QString ty_hou_ua_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[32]<<8) + readData[33]));
    QString ty_hou_ub_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%d", ((readData[34]<<8) + readData[35]));
    QString ty_hou_uc_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[36]<<8) + readData[37])/10.0);
    QString ty_hou_ia_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[38]<<8) + readData[39])/10.0);
    QString ty_hou_ib_buf = temp;

    memset(temp, 0, 20);
    sprintf(temp, "%.1f", ((readData[40]<<8) + readData[41])/10.0);
    ...
    QString ty_hou_gear_buf = QString::number((readData[43] >> 1) & 0x1f);


    QString ty_hou_cap_buf;
    cap_num = readData[43] >> 6;

    if(cap_num == 0)
        ty_hou_cap_buf = QString::fromLocal8Bit("电容1：未投-电容2：未投");
    else if(cap_num == 1)
        ty_hou_cap_buf = QString::fromLocal8Bit("电容1：已投-电容2：未投");
    else if(cap_num == 2)
        ty_hou_cap_buf = QString::fromLocal8Bit("电容1：未投-电容2：已投");
    else if(cap_num == 3)
        ty_hou_cap_buf = QString::fromLocal8Bit("电容1：已投-电容2：已投");
    ...
    if(readData[44] >> 1 == 1)
            ...
            last_motion_buf = QString::fromLocal8Bit("投切电容");
...
void YZTY::get_ala_rec_handle(int len)
{
    ...
    sprintf(temp, "%.1f", ((readData[26]<<8) + readData[27])/10.0);
    QString ic_buf = temp;
    QString gear_buf = QString::number((readData[28] >> 1) & 0x1f);

    QString cap_buf;
    int cap_num = readData[28] >> 6;
    ...
    else
    {
        ...
        if((device_ago_state & 0x20) == 0x20)
            device_ago_state_buf = device_ago_state_buf + QString::fromLocal8Bit("电容1故障 | ");
        if((device_ago_state & 0x40) == 0x40)
            device_ago_state_buf = device_ago_state_buf + QString::fromLocal8Bit("电容2故障 | ");
        ...
        if((device_ago_state & 0x4000) == 0x4000)
            device_ago_state_buf = device_ago_state_buf + QString::fromLocal8Bit("电容投切失败 | ");
    }
    ...
    else
    {
        ...
        if((device_state & 0x10) == 0x10)
            device_state_buf = device_state_buf + QString::fromLocal8Bit("开关故障 | ");
        if((device_state & 0x20) == 0x20)
            device_state_buf = device_state_buf + QString::fromLocal8Bit("电容1故障 | ");
        if((device_state & 0x40) == 0x40)
            device_state_buf = device_state_buf + QString::fromLocal8Bit("电容2故障 | ");
        ...
        if((device_state & 0x4000) == 0x4000)
            device_state_buf = device_state_buf + QString::fromLocal8Bit("电容投切失败 | ");
    }
...
void YZTY::on_select_cfg_clicked()
{
    ...
    if(cfgini.contains("device_code/code"))
    {
        ...
        ui->pf_up->setText(cfgini.value("qPara/pf_up").toString());
        ui->pf_down->setText(cfgini.value("qPara/pf_down").toString());
        ui->q_need->setText(cfgini.value("qPara/q_need").toString());
        ui->tc_delay->setText(cfgini.value("qPara/tc_delay").toString());
        ui->qc_delay->setText(cfgini.value("qPara/qc_delay").toString());
    }
}
